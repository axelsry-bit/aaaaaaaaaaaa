...
client.once('ready', () => {
  const statuses = [
    () => `Chilling with the owner`,
    () => `${client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0)} membres`,
    () => `${prefix}help pour page d'aide`,
  ]
  let i = 0
  setInterval(() => {
    client.user.setActivity(statuses[i](), {type: 'STREAMING', url: 'https://www.twitch.tv/astro_sry'})
    i = ++i % statuses.length
  }, 1e4)
})
...
client.on('message', message =>{
  if (message.content.startsWith("ressuscite")){
      message.send    
      const exampleEmbed = new Discord.MessageEmbed()
      .setDescription("Il est déja au ciel avec p'tit Jésus.")
      message.channel.send(exampleEmbed);
      console.log('répond à ressuscite');
